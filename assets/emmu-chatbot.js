...
(function () {
  ...
  function getResponse(intent, msg) {
    switch (intent) {
      ...
      case 'experience': {
        var html = '<b>Professional Experience (3 Roles)</b><br><br>';
        KB.experience.forEach(function(exp) {
          html += '<b>' + exp.role + '</b><br>';
          html += exp.company + ' — ' + exp.location + '<br>';
          html += '<em>' + exp.period + '</em><br>';
      ...
      case 'publications': {
        ...
        KB.publications.forEach(function(pub) {
          html += '<b>' + pub.title + '</b><br>';
          html += '<em>' + pub.status + '</em><br>';
          html += pub.authors + '<br>';
          html += pub.summary + '<br><br>';
        });
        return html;
      }
      ...
      case 'cv':
        return 'You can view and download Eman\'s updated ATS-compliant CV directly from the portfolio!<br><br>Click the <b>"Download Resume"</b> button at the top of the page, or <a href="resume.html" target="_blank">click here to open & save CV as PDF</a>.';

      case 'help':
        return '<b>Here is what I can tell you!</b><br><br>' +
          'About Eman — background & bio<br>' +
          'Education — university, CGPA, degree<br>' +
          'Skills — all technical skills & tools<br>' +
          'Projects — all 8 projects with details<br>' +
          'Experience — internships & work history<br>' +
          'Publications — research papers<br>' +
          'Certifications — all 6 certifications<br>' +
          'Leadership — IEEE, team lead roles<br>' +
          'Contact — email, phone, LinkedIn<br><br>' +
      ...
      case 'bye':
        return 'Goodbye! It was lovely chatting with you. Feel free to come back anytime to learn more about Eman. Have a wonderful day!';

      default:
        return 'Hmm, I\'m not sure about that one! I specialize in everything about <b>Eman Fatima\'s portfolio</b>. Try asking me about her skills, projects, experience, or how to contact her. Type <em>"help"</em> to see all topics!';
    }
  }
  ...
  function buildChatbot() {
    ...
    style.textContent = [
      ...
      '#emmu-fab-label{font-family:"Manrope",sans-serif;font-size:7.5px;font-weight:800;color:#fff;letter-spacing:0.06em;text-transform:uppercase;line-height:1;margin-top:1px;}',
      ...
      '#emmu-tooltip{position:fixed;bottom:108px;left:32px;background:linear-gradient(135deg,#8b1e2d,#c0392b);color:#fff;padding:10px 16px;border-radius:14px 14px 14px 4px;font-family:"Manrope",sans-serif;font-size:13px;font-weight:600;white-space:nowrap;box-shadow:0 8px 24px rgba(139,30,45,0.35);z-index:9999;pointer-events:none;animation:emmuSlideIn 0.4s cubic-bezier(0.16,1,0.3,1) forwards;}',
      '@keyframes emmuSlideIn{from{opacity:0;transform:translateY(10px);}to{opacity:1;transform:translateY(0);}}',
      '#emmu-window{position:fixed;bottom:112px;left:32px;width:374px;max-height:590px;background:var(--card,#ffffff);border:1px solid var(--border,#f0dde0);border-radius:22px;box-shadow:0 24px 70px rgba(139,30,45,0.2),0 4px 16px rgba(0,0,0,0.08);display:flex;flex-direction:column;z-index:9999;overflow:hidden;transform-origin:bottom left;animation:emmuWinIn 0.45s cubic-bezier(0.16,1,0.3,1) forwards;}',
      ...
      '.emmu-hinfo{flex:1;min-width:0;}',
      '.emmu-hname{font-family:"Fraunces",serif;font-size:14.5px;font-weight:700;color:#fff;line-height:1.2;}',
      '.emmu-hstatus{display:flex;align-items:center;gap:5px;font-size:10.5px;color:rgba(255,255,255,0.82);margin-top:2px;font-family:"Space Mono",monospace;}',
      ...
      '#emmu-msgs::-webkit-scrollbar{width:4px;}#emmu-msgs::-webkit-scrollbar-thumb{background:var(--border,#f0dde0);border-radius:4px;}',
      ...
      '.emmu-msg.user .emmu-msg-icon{background:var(--surface2,#fbf1f2);border:1px solid var(--border,#f0dde0);font-size:13px;}',
      '.emmu-bubble{max-width:80%;padding:10px 13px;border-radius:18px 18px 18px 4px;font-family:"Manrope",sans-serif;font-size:13px;line-height:1.65;color:var(--text,#25080f);background:var(--surface2,#fdf5f6);border:1px solid var(--border,#f0dde0);word-break:break-word;}',
      ...
      '.emmu-dot{width:7px;height:7px;border-radius:50%;background:var(--maroon2,#c0392b);opacity:0.5;animation:emmuTyping 1.2s ease-in-out infinite;}',
      ...
      '#emmu-quick{padding:8px 12px 10px;display:flex;flex-wrap:wrap;gap:6px;border-top:1px solid var(--border,#f0dde0);background:var(--card,#fff);flex-shrink:0;}',
      '.emmu-qbtn{padding:5px 11px;border-radius:100px;background:var(--surface2,#fdf5f6);border:1px solid var(--border2,#e3c3ca);color:var(--maroon2,#c0392b);font-size:11px;font-weight:700;cursor:pointer;font-family:"Manrope",sans-serif;transition:all 0.22s cubic-bezier(0.34,1.56,0.64,1);white-space:nowrap;}',
      ...
      '#emmu-input-area{padding:9px 13px 13px;display:flex;gap:8px;align-items:center;border-top:1px solid var(--border,#f0dde0);background:var(--card,#fff);flex-shrink:0;}',
      '#emmu-input{flex:1;padding:10px 14px;border:1.5px solid var(--border,#f0dde0);border-radius:100px;background:var(--input-bg,#fbf4f5);color:var(--text,#25080f);font-family:"Manrope",sans-serif;font-size:13px;outline:none;transition:border-color 0.2s,box-shadow 0.2s;}',
      '#emmu-input:focus{border-color:#c0392b;box-shadow:0 0 0 3px rgba(139,30,45,0.1);}',
      '#emmu-input::placeholder{color:var(--muted,#a2818a);}',
      '#emmu-send{width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#8b1e2d,#c0392b);border:none;color:#fff;font-size:15px;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:transform 0.25s cubic-bezier(0.34,1.56,0.64,1),box-shadow 0.25s;box-shadow:0 4px 14px rgba(139,30,45,0.35);flex-shrink:0;}',
      ...
      '[data-theme="dark"] #emmu-window{background:#280c14;border-color:#4a1a26;}',
      '[data-theme="dark"] .emmu-bubble{background:#320f19;border-color:#4a1a26;color:#fbeef0;}',
      '[data-theme="dark"] .emmu-qbtn{background:#320f19;border-color:#5e2130;}',
      '[data-theme="dark"] #emmu-quick,[data-theme="dark"] #emmu-input-area{background:#280c14;border-color:#4a1a26;}',
      '[data-theme="dark"] #emmu-input{background:rgba(255,255,255,0.04);border-color:#4a1a26;color:#fbeef0;}'
    ].join('');
    ...
    function appendMsg(text, isUser) {
      ...
      if (isUser) {
        iconEl.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" style="color:var(--maroon2,#c0392b)"><path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z"/></svg>';
      } else {
        iconEl.textContent = '🤖';
      }
    ...
    function openChat() {
      ...
      if (msgs.children.length === 0) {
        setTimeout(function() {
          appendMsg('Hey there! I\'m <b>Emmu</b> — Eman\'s AI portfolio assistant! I know <em>everything</em> about Eman Fatima — her skills, projects, research, experience and more. What would you like to know?', false);
        }, 300);
      }
